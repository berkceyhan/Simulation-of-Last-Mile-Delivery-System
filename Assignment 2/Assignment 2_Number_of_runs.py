...
#
# Lastly, the speed is changed to stochastic speed to represent the real world situation better.   
# You can check "5.2 Old functions modified to support stochastic speed" and also the "8.3 check_charge function" to see the changes 
#
# To code these 3 parts, both new functions are introduced, and also some existing functions are modified. All the changes will be explained in detail
# in the related parts of the code.
# 
# Additionally, with this script, you will have the chance to see the statistics of the maximum tour end time,
# and waiting times at the charging stations. Moreover, you will see how the number of runs are calculated.
# 
# 
# 
# 
...
num_van = 4 # !!!!! 4 is the optimal value, which is reached in the routing part. 
            # Please do not change it until you read the conclusion of the "Routing" section

# 2.2 Old Parameters
avg_speed = 30 #km/h - Average speed
warehouse_x = 35 # please do NOT change this parameter
warehouse_y = 35# please do NOT change this parameter
num_stations = 130
battery_size = [30,30,20,20] # you can play with battery sizes
...
#       8.1.2 New and modified attributes
    ...
    def check_charge(self):
        ...
        if stochastic_speed==1:
            self.speed=random.normalvariate(30,2)  # speed is now STOCHASTIC, updated in every step
...
while True:
    for i in range(50): # to make multiple simulations        
        task_end_time=[]
        ...
        task_end_time_multiple_runs.append(task_end_time)
        maximum=max(task_end_time)
        max_task_end_time.append(maximum)
        MEAN.append(statistics.mean(max_task_end_time))
        if i>0:
            SAMPLE_VARIANCE.append(statistics.variance(max_task_end_time))
            SAMPLE_MEAN_VARIANCE.append(statistics.variance(MEAN))
            
            
    
        for a in model.schedule_vans.agents: # to keep the track of the waiting times of the vans at the charging stations
            waiting_times_van= np.append(waiting_times_van,a.waiting_times_van)
    
        #waiting times at the charging station statistics
        if len(waiting_times_van)>0:
            MEAN_WT.append(statistics.mean(waiting_times_van))

        if len(waiting_times_van)>1:    
            VARIANCE_WT.append(statistics.variance(waiting_times_van))
            
    #calculating c
    s=statistics.variance(max_task_end_time)
    quantile=scipy.stats.t.ppf(1-alpha/2,N-1)
    c=quantile*(s/N)**0.5
    print("N=",N,"c/mean=", c/MEAN[-1])
    
    if c<=0.005*statistics.mean(max_task_end_time):
        break
    else:
        N=N+50

print("\n***Parameters used for this experiment are:***")
if charging_policy==1:
    print("Charging policy is: The New Charging Policy")
elif charging_policy==0:
    print("Charging policy is:The Base Case Charging Policy")
print("Battery sizes used for each van are:",battery_size)
print("\n***Number of Runs Calculation Depending On Max Tour End Time Statistics***")
print("c=",c)
print("Mean=",MEAN[-1])
print("c/MEAN=",c/MEAN[-1])
print("Number of runs=", N)
# Max Tour End Time: The maximum of the tour end times of all the vans in a single run "
print("\n**Max Tour End Time Statistics***")
print("Mean=",MEAN[-1])
print("Sample variance=",SAMPLE_VARIANCE[-1])
print("Variance of the sample mean=",SAMPLE_VARIANCE[-1]/N)

z=(480-MEAN[-1])/(SAMPLE_VARIANCE[-1]**0.5)
P_OVERTIME=1-scipy.stats.norm.cdf(z)
print("P(max throughput time>480)=",P_OVERTIME)
# print("\nwaiting time of the vans matrix:\n",waiting_times_van)
# print("multiple runs task end time is:",task_end_time_multiple_runs)
# print("task end times of the vans:",task_end_time)
# print("max task end time is:",max_task_end_time)
print("\n***Waiting Times At The Charging Station Statistics:***")
if len(MEAN_WT)>0 and len(VARIANCE_WT)>0:
    print("Mean of the waiting times of vans at the charging stations=",MEAN_WT[-1])
    print("Variance of the waiting times of vans at the charging stations=",VARIANCE_WT[-1])
else:
    print("The vans did not need any charging in these runs")
#print("waiting times van=",waiting_times_van)
...
plt.legend()
plt.show()
plt.plot(SAMPLE_MEAN_VARIANCE,label="SAMPLE MEAN VARIANCE")
plt.legend()
plt.show()
plt.hist(max_task_end_time,label="max task end time histogram")
plt.legend()
plt.show()
plt.hist(waiting_times_van,label="waiting times van histogram")
plt.legend()
