...
# 1. Summary 
# 2. Parameters
#    2.1 Parameters for Routing (NEW)
...
#           4. Introducing battery sizes which can change for different vans
#           5. Introducing van capacities and showing current load of the vans
#
...
#
# Moreover, the speed is changed to stochastic speed to represent the real world situation better.   
...
# 
# You can go to section2 of this script "Parameters" to play with them.
# 
# 
# 
# 
...
num_customers = 60
num_van = 4 # In the routing code, since 4 vans give optimal solution
            # if you enter num_van>=5, it will still give you the solution
            # with 4 vans. So please try it with num_vans<=4
...
num_stations = 130
battery_size = [30,30,20,20] #kwh, [van_0,van_1,van_2,van_3] respectively
                             # you can play with this parameter to see effect of the battery size
                             # on the tour end times of the vans  
...
charging_policy=1 # You can change the policy and check the waiting times of the vans at the charging stations
safety_energy_level=1.15 # when calculating needed_energy, 
                         # this safety level charges extra amount of battery, due to speed stochasticity causing
                         # also stochasticity in speed consumption. 
...
import pandas as pd
from ortools.constraint_solver import routing_enums_pb2 # pip install --upgrade --user ortools 
from ortools.constraint_solver import pywrapcp          # pip install --upgrade --user ortools 
            ...
            if TTT[i]-CWT[i]>=25: 
                if CWT[i]/45*25+CWT[i]>TTT[i]: 
                    ...
                    EWT.append(15.46)  # 15.46 is determined by running simulations specificly to identify
                                        ...
                                        # This value is used only when the charging station is very far away
                                        # from the van's location, making short-term probability calculations
                                        # imposibble, so we give an approximate value.
                ...
                elif CWT[i]<=45:    # refer to the presentation for the explanation of this case
...
# 5.6 "needed_energy": 
# Calculating the EXTRA energy needed for a van for its all tour, given the total distance it travels
# which is calculated easily after determining the routes, and also given its battery size
...
capacity=[]
average_order=77
for el in range(len(van_routes)):
    capacity.append(average_order*len(van_routes[el]))
...
#       8.1.2 New and modified attributes
        ...
        # capacity
        self.capacity=capacity[int(self.id_van[4:])]
...
#       8.3.2 Base case charging policy     
    ...
    def move(self):
        ...
        #print("Remaining load (kg)  of the van",self.capacity)
...
task_end_time=[]
max_task_end_time=[]  
waiting_times_van=np.empty(0)  # to keep the track of the waiting times of the vans for the charging station queues

for i in range(1): # to make multiple simulations
    task_end_time=[]
...
print("\nWaiting times of the vans at the charging station queues:\n",waiting_times_van)
...
for i in range(len(XX)):
    ...
    if i==len(XX)-1:
        plt.plot(XX_Charging[i], YY_Charging[i], "s", color="b", label='Charging Stations Visited')
    else:
